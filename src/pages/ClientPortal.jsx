...
const ClientPortal = () => {
  ...
  useEffect(() => {
    // Mock notifications
    setNotifications([
      { id: 1, type: 'info', message: 'Your project milestone has been completed', time: '2 hours ago', read: false },
      { id: 2, type: 'success', message: 'Invoice #INV-001 has been paid', time: '1 day ago', read: true },
      { id: 3, type: 'warning', message: 'Subscription renewal required', time: '3 days ago', read: false }
    ]);

    // Mock projects
    setProjects([
      { id: 1, name: 'HR-IMS Implementation', status: 'completed', progress: 100, client: 'TechCorp', deadline: '2024-01-15' },
      { id: 2, name: 'TrackIT Migration', status: 'in-progress', progress: 75, client: 'LogiSolutions', deadline: '2024-02-28' },
      { id: 3, name: 'API Integration', status: 'pending', progress: 10, client: 'FinancePro', deadline: '2024-03-15' }
    ]);

    // Mock invoices
    setInvoices([
      { id: 'INV-001', date: '2024-01-01', amount: 25000, status: 'paid', dueDate: '2024-01-15' },
      { id: 'INV-002', date: '2024-02-01', amount: 15000, status: 'pending', dueDate: '2024-02-15' },
      { id: 'INV-003', date: '2024-03-01', amount: 35000, status: 'pending', dueDate: '2024-03-15' }
    ]);
  }, []);
  ...
  const stats = [
    { label: 'Active Projects', value: projects.filter(p => p.status !== 'completed').length, change: '+2', icon: HiChartBar },
    { label: 'Total Invoices', value: invoices.length, change: '+1', icon: HiCurrencyDollar },
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {stats.map((stat, index) => {
          ...
          return (
            <div key={index} className="bg-white dark:bg-dark-800 rounded-xl p-6 shadow-soft border border-gray-100 dark:border-dark-700">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-white dark:bg-dark-800 rounded-2xl p-6 shadow-soft border border-gray-100 dark:border-dark-700">
          <h3 className="text-xl font-bold mb-6 text-gray-900 dark:text-white">Recent Projects</h3>
          <div className="space-y-4">
            {projects.slice(0, 3).map((project) => (
              <div key={project.id} className="flex items-center justify-between p-4 bg-gray-50 dark:bg-dark-700 rounded-lg">
        ...
        {/* Invoices */}
        <div className="bg-white dark:bg-dark-800 rounded-2xl p-6 shadow-soft border border-gray-100 dark:border-dark-700">
          <h3 className="text-xl font-bold mb-6 text-gray-900 dark:text-white">Recent Invoices</h3>
          <div className="space-y-4">
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {projects.map((project) => (
          <div key={project.id} className="bg-white dark:bg-dark-800 rounded-2xl p-6 shadow-soft border border-gray-100 dark:border-dark-700">
            <div className="flex items-center justify-between mb-4">
            ...
            <p className="text-gray-600 dark:text-gray-400 text-sm mb-4">Client: {project.client}</p>
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-gray-50 dark:bg-dark-700">
              <tr>
                <th className="px-6 py-4 text-left text-sm font-medium text-gray-900 dark:text-white">Invoice ID</th>
                <th className="px-6 py-4 text-left text-sm font-medium text-gray-900 dark:text-white">Date</th>
                <th className="px-6 py-4 text-left text-sm font-medium text-gray-900 dark:text-white">Amount</th>
                <th className="px-6 py-4 text-left text-sm font-medium text-gray-900 dark:text-white">Status</th>
                <th className="px-6 py-4 text-left text-sm font-medium text-gray-900 dark:text-white">Due Date</th>
              ...
              {invoices.map((invoice) => (
                <tr key={invoice.id} className="hover:bg-gray-50 dark:hover:bg-dark-700">
                  ...
                  <td className="px-6 py-4 text-sm">
                    <button className="text-blue-600 dark:text-blue-400 hover:underline mr-3">View</button>
                    <button className="text-gray-600 dark:text-gray-400 hover:underline">Download</button>
                  </td>
                </tr>
              ))}
            </tbody>
  ...
  const renderDocuments = () => (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {[
          { name: 'Project Proposal.pdf', type: 'pdf', size: '2.4 MB', date: '2024-01-10' },
          { name: 'Contract Agreement.docx', type: 'doc', size: '1.8 MB', date: '2024-01-08' },
          { name: 'Technical Specs.pdf', type: 'pdf', size: '3.2 MB', date: '2024-01-05' },
          { name: 'Meeting Notes.txt', type: 'txt', size: '0.5 MB', date: '2024-01-03' },
          { name: 'Project Timeline.xlsx', type: 'xls', size: '1.1 MB', date: '2024-01-01' },
          { name: 'Security Audit.pdf', type: 'pdf', size: '4.7 MB', date: '2023-12-28' }
        ].map((doc, index) => (
          <div key={index} className="bg-white dark:bg-dark-800 rounded-2xl p-6 shadow-soft border border-gray-100 dark:border-dark-700">
            <div className="flex items-center mb-4">
              <div className="w-12 h-12 bg-gradient-to-br from-blue-500 to-indigo-500 rounded-lg flex items-center justify-center mr-4">
                {doc.type === 'pdf' && <span className="text-white font-bold text-xs">PDF</span>}
                {doc.type === 'doc' && <span className="text-white font-bold text-xs">DOC</span>}
                {doc.type === 'xls' && <span className="text-white font-bold text-xs">XLS</span>}
                {doc.type === 'txt' && <span className="text-white font-bold text-xs">TXT</span>}
              </div>
              <div className="flex-1">
              ...
              <div className="bg-white dark:bg-dark-800 rounded-2xl p-6 shadow-soft border border-gray-100 dark:border-dark-700 sticky top-8">
                <div className="flex items-center mb-6">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500 dark:text-gray-500">{user.company}</p>
                  </div>
                </div>
                ...
                <div className="mt-8 pt-6 border-t border-gray-200 dark:border-dark-600">
                  <div className="space-y-3">
                    ...
                    <div className="flex items-center justify-between text-sm">
                      <span className="text-gray-600 dark:text-gray-400">Account Since</span>
                      <span className="font-medium text-gray-900 dark:text-white">{user.joinDate}</span>
                    </div>
                  </div>
                </div>
              </div>

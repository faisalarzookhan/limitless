...
const AuralisPersonalizationEngine = ({ children, userId = 'anonymous' }) => {
  ...
  const detectIndustry = (behavior) => {
    if (behavior.pagesVisited?.includes('/hr-ims') || behavior.pagesVisited?.includes('/hr')) {
      return 'Human Resources';
    } else if (behavior.pagesVisited?.includes('/trackit') || behavior.pagesVisited?.includes('/project')) {
      return 'Project Management';
    } else if (behavior.pagesVisited?.includes('/ecommerce') || behavior.pagesVisited?.includes('/retail')) {
      return 'E-commerce/Retail';
    } else if (behavior.pagesVisited?.includes('/healthcare') || behavior.pagesVisited?.includes('/medical')) {
      return 'Healthcare';
    } else if (behavior.pagesVisited?.includes('/finance') || behavior.pagesVisited?.includes('/banking')) {
  ...
  useEffect(() => {
    const trackBehavior = () => {
      ...
      const detectedIndustry = detectIndustry({
        pagesVisited: [...(userProfile.behavior.pagesVisited || []), currentPath],
        referrer
      });
  ...
  useEffect(() => {
    const calculateEngagementLevel = () => {
      ...
      const timeOnSite = behavior.timestamp ? Date.now() - behavior.timestamp : 0;
      
      if (pagesCount >= 5 && timeOnSite > 300000) { // 5+ pages, 5+ minutes
        return 'high';
      } else if (pagesCount >= 3 && timeOnSite > 120000) { // 3+ pages, 2+ minutes
        return 'medium';
      } else {
        return 'low';
      }

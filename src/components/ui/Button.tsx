...
const Button = forwardRef<HTMLButtonElement, ButtonProps>(
  (
    ...
    const variantClasses = {
      primary:
        'bg-gradient-primary text-white hover:shadow-lg hover:shadow-primary-500/30 focus:ring-primary-500',
      secondary:
        'bg-gray-100 dark:bg-dark-800 text-gray-900 dark:text-white hover:bg-gray-200 dark:hover:bg-dark-700 focus:ring-gray-500',
      outline:
      ...
      ghost:
        'bg-transparent text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-dark-800 focus:ring-gray-500',
    ...
    useEffect(() => {
      if (process.env.NODE_ENV === 'development') {
        if (!ariaLabel && !children && !icon) {
          console.warn(
            'Button component missing accessible name (aria-label, children, or icon)'
          );
        }
      }

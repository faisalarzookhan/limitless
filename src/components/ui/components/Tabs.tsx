...
const Tabs = forwardRef<HTMLDivElement, TabsProps>(({
  ...
  const handleKeyDown = (event: React.KeyboardEvent, index: number) => {
    switch (event.key) {
      ...
      case 'ArrowRight':
        event.preventDefault();
        setActiveTab(prev => Math.min(React.Children.count(children) - 1, prev + 1));
        break;
      case 'Home':
        event.preventDefault();
        setActiveTab(0);
  ...
  const tabsContainerClasses = variant === 'pill'
  ...
  return (
    <div 
      ...
      {...props}
    >
      <div className={tabsContainerClasses} role="tablist" aria-orientation="horizontal">
        {enhancedChildren}
      </div>
    </div>
  );

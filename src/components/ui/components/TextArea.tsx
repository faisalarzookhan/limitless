...
const TextArea = forwardRef<HTMLTextAreaElement, TextAreaProps>(({
  ...
  const baseTextAreaClasses = 'w-full rounded-lg border bg-white dark:bg-dark-800 text-gray-900 dark:text-gray-100 transition-all duration-300 focus:outline-none focus:ring-2 resize-none';
  ...
  return (
    <div className="w-full">
      ...
      />
      {error && (
        <p id={`${textAreaId}-error`} className="text-sm text-red-600 dark:text-red-400 mt-1" role="alert">
          {error}
        </p>
      )}
    </div>

...
const Select = forwardRef<HTMLSelectElement, SelectProps>(({
  ...
  const baseSelectClasses = 'w-full rounded-lg border bg-white dark:bg-dark-800 text-gray-900 dark:text-gray-100 transition-all duration-300 focus:outline-none focus:ring-2 appearance-none pr-10';
  ...
  return (
    <div className="w-full relative">
      ...
      {error && (
        <p id={`${selectId}-error`} className="text-sm text-red-600 dark:text-red-400 mt-1" role="alert">
          {error}
        </p>
      )}
    </div>

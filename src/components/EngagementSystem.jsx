...
const EngagementSystem = ({ 
  ...
  useEffect(() => {
    if (userType === 'auditor' && userData.email && results.limitlessScore !== undefined) {
      // Track auditor interaction
      const interaction = engagementService.trackAuditorInteraction(userData, results);
      if (onEngagementTracked) {
        onEngagementTracked(interaction);
      }
    } else if (userType === 'sandbox' && results.sandboxId) {
      // Track sandbox usage
      const interaction = engagementService.trackSandboxUsage(results.sandboxId, results.usageData);
      if (onEngagementTracked) {
        onEngagementTracked(interaction);
      }
    }

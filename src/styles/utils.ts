...
export const getFontSize = (size: string): string => {
  return designSystem.typography.fontSize[size as keyof typeof designSystem.typography.fontSize] || size;
};

// Helper function to get font family values
export const getFontFamily = (family: string): string => {
  const fontFamily = designSystem.typography.fontFamily[family as keyof typeof designSystem.typography.fontFamily];
  return Array.isArray(fontFamily) ? fontFamily.join(', ') : fontFamily || family;
};

// Helper function to get border radius values
export const getBorderRadius = (radius: string): string => {
  return designSystem.borderRadius[radius as keyof typeof designSystem.borderRadius] || radius;
};

// Helper function to get box shadow values
export const getBoxShadow = (shadow: string): string => {
  return designSystem.boxShadow[shadow as keyof typeof designSystem.boxShadow] || shadow;
};
...
export const getZIndex = (index: string | number): string => {
  ...
  return designSystem.zIndex[index as keyof typeof designSystem.zIndex] || index;
};

// Helper function to get breakpoint values
export const getBreakpoint = (breakpoint: string): string => {
  return designSystem.breakpoints[breakpoint as keyof typeof designSystem.breakpoints] || breakpoint;
};

// Helper function to get transition values
export const getTransition = (duration: string, easing: string = 'default'): string => {
  const durationValue = designSystem.transitions.duration[duration as keyof typeof designSystem.transitions.duration] || duration;
  const easingValue = designSystem.transitions.easing[easing as keyof typeof designSystem.transitions.easing] || easing;
  return `${durationValue} ${easingValue}`;
};

// Helper function to get gradient values
export const getGradient = (gradient: string): string => {
  return designSystem.gradients[gradient as keyof typeof designSystem.gradients] || gradient;
};

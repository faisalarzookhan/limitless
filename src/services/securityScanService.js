...
class SecurityScanService {
  ...
  async scanSSLCertificate(url) {
    ...
    const now = new Date();
    const validFrom = new Date(now.getTime() - Math.random() * 365 * 24 * 60 * 60 * 1000); // Random date in the past year
    const validTo = new Date(now.getTime() + (Math.random() * 365 + 90) * 24 * 60 * 60 * 1000); // Valid for 90 days to 1 year
  ...
  getRandomSSLVersion() {
    const versions = ['TLS 1.3', 'TLS 1.2', 'TLS 1.1', 'TLS 1.0', 'SSL 3.0', 'SSL 2.0'];
  ...
  generateCertificateChain() {
    ...
    for (let i = 0; i < chainLength; i++) {
      chain.push({
        ...
        validFrom: new Date(Date.now() - Math.random() * 30 * 24 * 60 * 60 * 1000).toISOString(),
  ...
  generateSSLIssues() {
    ...
    if (Math.random() > 0.8) issues.push('Certificate uses weak signature algorithm');
  ...
  generateHeaderIssues() {
    ...
    if (Math.random() > 0.5) issues.push('Missing X-Content-Type-Options header');
    if (Math.random() > 0.6) issues.push('Missing XSS protection header');
    if (Math.random() > 0.5) issues.push('Missing Strict-Transport-Security header');
    if (Math.random() > 0.7) issues.push('Missing Content-Security-Policy header');
    if (Math.random() > 0.6) issues.push('Missing Referrer-Policy header');
    if (Math.random() > 0.65) issues.push('Missing Cross-Origin Resource Policy header');
    if (Math.random() > 0.7) issues.push('Missing Cross-Origin Embedder Policy header');
    if (Math.random() > 0.65) issues.push('Missing Cross-Origin Opener Policy header');
    
    // CSP-specific issues
    if (Math.random() > 0.8) issues.push('Content-Security-Policy: Wildcard directives detected');
    if (Math.random() > 0.75) issues.push('Content-Security-Policy: Unsafe inline scripts allowed');
    if (Math.random() > 0.85) issues.push('Content-Security-Policy: Unsafe eval allowed');
    if (Math.random() > 0.7) issues.push('Content-Security-Policy: Overly permissive image sources');
  ...
  generateRecommendations(sslData, headerData) {
    ...
    if (!sslData.hsts) {
      recommendations.push('Implement HTTP Strict Transport Security (HSTS) to force HTTPS connections');
    }
    ...
    if (!headerData.xssProtection) {
      recommendations.push('Enable XSS protection headers to prevent cross-site scripting');
    }
    
    if (sslData.sslGrade === 'F' || sslData.sslGrade === 'D') {
      recommendations.push('Upgrade your SSL certificate to improve security grade');
    }

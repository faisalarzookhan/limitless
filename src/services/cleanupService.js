...
class CleanupService {
  ...
  createCleanupJob(jobConfig) {
    const job = {
      id: jobConfig.id || `cleanup-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
      name: jobConfig.name,
      rule: jobConfig.rule,
  ...
  calculateNextRun(schedule) {
    ...
    if (schedule === '0 1 * * *') { // Daily at 1 AM
      nextTime.setDate(nextTime.getDate() + 1);
      nextTime.setHours(1, 0, 0, 0);
    } else if (schedule === '0 2 * * *') { // Daily at 2 AM
      nextTime.setDate(nextTime.getDate() + 1);
  ...
  async executeCleanupJob(jobId) {
    ...
    try {
      ...
      console.log(`Cleanup job completed: ${job.name}, ${cleanupResult.filesRemoved} files removed`);
  ...
  async cleanupLocation(location, rule) {
    ...
    const mockFiles = [
      { name: 'temp1.tmp', size: 1024, timestamp: Date.now() - (10 * 24 * 60 * 60 * 1000) }, // 10 days old
      { name: 'temp2.tmp', size: 2048, timestamp: Date.now() - (5 * 24 * 60 * 60 * 1000) }, // 5 days old
      { name: 'old.log', size: 4096, timestamp: Date.now() - (35 * 24 * 60 * 60 * 1000) }  // 35 days old
    ];
    ...
    console.log(`Removed ${filesToRemove.length} files from ${location}, freed ${sizeFreed} bytes`);
  ...
  getCleanupStatus() {
    return {
      ...
      enabledJobs: Array.from(this.cleanupJobs.values()).filter(job => job.enabled).length,
      totalCleanups: this.cleanupHistory.length,
      successfulCleanups: this.cleanupHistory.filter(c => c.status === 'success').length,
  ...
  async performSystemCleanup() {
    ...
    const results = {
      files: await this.performCleanup(this.cleanupRules.get('temp-files'), ['temp']),
      logs: await this.performCleanup(this.cleanupRules.get('log-files'), ['logs']),

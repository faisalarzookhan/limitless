...
class VPCIsolationService {
  ...
  async createIsolatedVPC(sandboxId, config = {}) {
    ...
    const securityGroupId = await this.createDefaultSecurityGroup(vpcId, sandboxId);
  ...
  getVpcConfig(sandboxId) {
    ...
    return {
      ...
      securityGroups: networkInfo.securityGroups.map(id => this.securityGroups.get(id)),
  ...
  async createAdditionalSubnet(vpcId, availabilityZone = null) {
    ...
    const subnetConfig = {
      ...
      vpcId,
      state: 'pending',
      cidrBlock: this.calculateNextSubnetCidr(vpc.cidrBlock, this.subnetConfigs.size),
      availableIpAddressCount: 251,
      availabilityZone: availabilityZone || `us-east-1${String.fromCharCode(97 + (this.subnetConfigs.size % 3))}`,
  ...
  async isolateVpc(vpcId, options = {}) {
    ...
    if (routeTable) {
      routeTable.routes = routeTable.routes.filter(route => route.gatewayId !== 'igw-*');
    }
    ...
    if (options.restrictiveAcl) {
      ...
      if (networkAcl) {
        ...
        networkAcl.entries = networkAcl.entries.filter(entry => 
          !(entry.ruleAction === 'allow' && entry.egress === true)
        );
  ...
  getIsolationReport(sandboxId) {
    ...
    return {
      ...
      securityCompliance: {
        defaultSecurityGroupRules: this.checkDefaultSecurityGroupRules(networkInfo.securityGroups[0]),
        networkAclRules: this.checkNetworkAclRules(networkInfo.networkAcls[0]),
        routeTableSecurity: this.checkRouteTableSecurity(networkInfo.routeTables[0])
      },
      status: 'isolated'
    };
  }
  ...
  checkDefaultSecurityGroupRules(sgId) {
    ...
    const hasIngressAll = sg.ipPermissions.some(perm => 
      perm.ipRanges && perm.ipRanges.some(range => range.cidrIp === '0.0.0.0/0')
    );
  ...
  checkNetworkAclRules(aclId) {
    ...
    const overlyPermissiveRules = acl.entries.filter(entry => 
  ...
  checkRouteTableSecurity(rtbId) {
    ...
    const hasInternetRoute = rtb.routes.some(route => 
  ...
  async cleanupVpcResources(sandboxId) {
    const networkInfo = this.sandboxNetworks.get(sandboxId);
    if (!networkInfo) {
      return { success: true, message: 'No network resources found for sandbox' };
    }
  ...
  validateNetworkConfig(config) {
    ...
    if (config.availabilityZone && !this.isValidAvailabilityZone(config.availabilityZone)) {

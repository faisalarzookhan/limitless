...
class ReportGenerationService {
  ...
  addTitlePage(doc, url, email) {
    ...
    doc.text('Technical Debt & Opportunity Report', pageWidth / 2, 100, {
      align: 'center',
    });
    ...
    doc.text(`Date: ${new Date().toLocaleDateString()}`, pageWidth / 2, 160, {
      align: 'center',
    });
    ...
    doc.setFontSize(10);
    doc.text(
      'Confidential - Limitless Infotech Solutions',
      pageWidth / 2,
      pageHeight - 20,
      { align: 'center' }
    );
  ...
  addExecutiveSummary(doc, auditResults) {
    ...
    if (auditResults.limitlessScore >= 80) {
      scoreInterpretation =
        'Excellent - Your website is performing at an elite level';
    } else if (auditResults.limitlessScore >= 60) {
      scoreInterpretation =
        'Good - Your website has solid performance with room for improvement';
    } else {
      scoreInterpretation =
        'Needs Attention - Significant improvements needed for optimal performance';
    }
    ...
    doc.autoTable({
      ...
      body: [
        [
          ...
          `${auditResults.performance.lcp.toFixed(0)}ms`,
          this.getPerformanceStatus(auditResults.performance.lcp, 2500),
        ],
        [
          'First Contentful Paint (FCP)',
          `${auditResults.performance.fcp.toFixed(0)}ms`,
          this.getPerformanceStatus(auditResults.performance.fcp, 1800),
        ],
        [
          'Cumulative Layout Shift (CLS)',
          auditResults.performance.cls.toFixed(3),
          this.getPerformanceStatus(auditResults.performance.cls, 0.1),
        ],
        [
          'First Input Delay (FID)',
          `${auditResults.performance.fid.toFixed(0)}ms`,
          this.getPerformanceStatus(auditResults.performance.fid, 100),
        ],
        [
          'Speed Index',
          `${auditResults.performance.si.toFixed(0)}ms`,
          this.getPerformanceStatus(auditResults.performance.si, 3400),
        ],
      ],
      theme: 'grid',
      styles: { fontSize: 10 },
    ...
    doc.autoTable({
      ...
      body: [
        [
          ...
          `${auditResults.seo.issues.length} issues`,
        ],
      ],
      theme: 'grid',
      styles: { fontSize: 10 },
  ...
  addSecurityFindings(doc, auditResults) {
    ...
    doc.autoTable({
      ...
      body: [
        [
          'SSL Certificate',
          auditResults.security.sslValid ? 'Valid' : 'Invalid',
          `Grade: ${auditResults.security.grade}`,
        ],
        ['HTTPS Enforcement', 'Enabled', 'SSL/TLS configured'],
        [
          'HSTS Headers',
          auditResults.security.hsts ? 'Present' : 'Missing',
          'HTTP Strict Transport Security',
        ],
        [
          'Security Headers',
          `${4 - auditResults.security.headerIssues.length}/7 present`,
          'X-Frame-Options, etc.',
        ],
      ],
      theme: 'grid',
  ...
  addSolutionPath(doc, auditResults) {
    ...
    doc.setTextColor(0, 0, 0);
    doc.text(
      'Contact Limitless Infotech Solutions today for a customized solution.',
      20,
      yPosition
    );
    ...
    yPosition += 10;
    doc.text(
      'Email: info@limitlessinfotech.com | Phone: +91 77109 09492',
      20,
      yPosition
    );
  }
  ...
  async sendReport(reportBlob, email, url) {
    ...
    return {
      success: true,
      message:
        'Report generated and download started. In production, this would be sent via email.',
      downloadUrl,
    };
  }

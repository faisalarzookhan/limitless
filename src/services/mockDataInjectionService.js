...
class MockDataInjectionService {
  ...
  generateProjects(count, industry) {
    ...
    return Array.from({ length: count }, (_, i) => ({
      ...
      status:
        this.projectStatuses[
          Math.floor(Math.random() * this.projectStatuses.length)
        ],
      ...
      priority:
        this.taskPriorities[
          Math.floor(Math.random() * this.taskPriorities.length)
        ],
      category: industry,
      createdAt: new Date(
  ...
  generateTasks(count, projects) {
    ...
    return Array.from({ length: count }, (_, i) => {
      ...
      return {
        ...
        status:
          this.taskStatuses[
            Math.floor(Math.random() * this.taskStatuses.length)
          ],
        priority:
          this.taskPriorities[
            Math.floor(Math.random() * this.taskPriorities.length)
  ...
  generateUsers(count, industry) {
    ...
    return Array.from({ length: count }, (_, i) => ({
      ...
      status:
        this.userStatuses[Math.floor(Math.random() * this.userStatuses.length)],
      hireDate: new Date(
  ...
  generatePerformanceReviews(count, employees) {
    return Array.from({ length: count }, (_, i) => ({
      ...
      areasForImprovement: this.generateSkills(
  ...
  generateAttendanceRecords(count, employees) {
    return Array.from({ length: count }, (_, i) => {
      ...
      const status = ['Present', 'Absent', 'Late', 'Half Day', 'Leave'][
        Math.floor(Math.random() * 5)
      ];
      ...
      return {
        ...
        checkIn:
          status === 'Absent' || status === 'Leave'
            ? null
            : `${Math.floor(Math.random() * 4) + 7}:${String(Math.floor(Math.random() * 60)).padStart(2, '0')}`,
        checkOut:
          status === 'Absent' || status === 'Leave'
            ? null
            : `${Math.floor(Math.random() * 4) + 15}:${String(Math.floor(Math.random() * 60)).padStart(2, '0')}`,
        hoursWorked:
          status === 'Absent' || status === 'Leave' ? 0 : Math.random() * 8 + 4,
        status: status,
        notes:
          status === 'Late'
            ? 'Traffic delay'
  ...
  generateTrainingRecords(count, employees) {
    ...
    return Array.from({ length: count }, (_, i) => ({
      ...
      provider: ['Internal', 'External', 'Online'][
        Math.floor(Math.random() * 3)
      ],
      startDate: new Date(
        2024,
      ...
      certificate:
        Math.random() > 0.7
          ? `CERT-${Math.floor(Math.random() * 10000)}`
          : null,
  ...
  generateCertifications(count) {
    ...
    while (
      uniqueCerts.size < count &&
      uniqueCerts.size < certifications.length
    ) {
      uniqueCerts.add(
  ...
  async injectMockData(
    ...
    console.log(
      `Injecting mock data for ${systemType} into sandbox ${sandboxId}`
    );

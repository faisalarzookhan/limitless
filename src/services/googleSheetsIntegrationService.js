...
class GoogleSheetsIntegrationService {
  ...
  async initialize() {
    if (!this.webhookEnabled && !this.simulateWebhook) {
      console.warn('Google Sheets integration not configured. Please set REACT_APP_GOOGLE_SHEETS_WEBHOOK_URL');
      return false;
    }
  ...
  formatLeadForSheet(leadData) {
    return {
      ...
      requirements: Array.isArray(leadData.requirements) ? leadData.requirements.join('; ') : '',
      ...
      notes: Array.isArray(leadData.notes) ? leadData.notes.map(n => n.content).join('; ') : '',
  ...
  async batchSyncToSheet(records, recordType) {
    ...
    for (const record of records) {
      try {
        ...
        results.push({ recordId: record.id, success: true, result: syncResult });
        successCount++;
      } catch (error) {
    ...
    switch (sheetName) {
      case 'leads':
        return {
          ...
          majorDimension: 'ROWS',
          values: [
            ['Timestamp', 'Name', 'Email', 'Company', 'Phone', 'Industry', 'Status', 'Priority', 'Score'],
            [new Date().toISOString(), 'John Doe', 'john@company.com', 'Company Inc', '555-1234', 'Technology', 'New', 'High', 85],
            [new Date().toISOString(), 'Jane Smith', 'jane@company.com', 'Tech Corp', '555-5678', 'Finance', 'Contacted', 'Medium', 72]
          ]
        };
      case 'inquiries':
        return {
          ...
          majorDimension: 'ROWS',
          values: [
            ['Timestamp', 'Name', 'Email', 'Interest', 'Message', 'Status', 'Priority'],
            [new Date().toISOString(), 'Mike Johnson', 'mike@client.com', 'Custom Development', 'Interested in enterprise solution', 'New', 'High'],
            [new Date().toISOString(), 'Sarah Wilson', 'sarah@client.com', 'Migration', 'Need help with cloud migration', 'In Progress', 'Medium']
          ]
        };
      default:
        return {
  ...
  async updateRecordInSheet(sheetName, recordId, updates) {
    if (this.simulateWebhook) {
      console.log(`Simulated update to ${sheetName} for record ${recordId}:`, updates);
      return {
        success: true,
    ...
    throw new Error('Update functionality requires real Google Sheets API implementation');
  }

  // Delete record from Google Sheets
  async deleteRecordFromSheet(sheetName, recordId) {
    if (this.simulateWebhook) {
      console.log(`Simulated deletion from ${sheetName} for record ${recordId}`);
      return {
        success: true,
    ...
    throw new Error('Delete functionality requires real Google Sheets API implementation');
  }
  ...
  async createSheet(sheetName) {
    ...
    throw new Error('Create sheet functionality requires real Google Sheets API implementation');
  }
